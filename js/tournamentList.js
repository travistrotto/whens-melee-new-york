...
function tournamentList(tournaments) {
    ...
    tournaments.forEach(tournament => {
        ...
        if (tournament.events.length > 0 && tournament.events[0].images && tournament.events[0].images.length > 0) {
            const imagesContainer = document.createElement('div');
            imagesContainer.classList.add('images-container');

            tournament.events[0].images.forEach(image => {
                const imageElement = document.createElement('img');
                imageElement.src = image.url; // Assuming image.url contains the image URL
                imagesContainer.appendChild(imageElement);
            });

            tournamentDiv.appendChild(imagesContainer);
        }
        ...
        tournamentDate.style.display = "inline"; // Set display to inline
        if (tournament.events.length > 0) {
            const event = tournament.events[0]; // Get the first event
            ...
            eventItem.style.display = "inline"; // Set display to inline
        ...
        tournamentDetailsLink.style.display = "inline"; // Set display to inline
        ...
        registerLink.style.display = "inline"; // Set display to inline
        tournamentDiv.appendChild(registerLink);


        // List of brackets being held at that event
        ...
        tournament.events.forEach(event => {
            ...
            // eventItem.innerHTML += ` ${event.name} - Start: ${new Date(event.startAt * 1000).toLocaleDateString()}, Registered: ${event.numEntrants || 'N/A'}`;
